...
function removeLeadingAndTrailingWhitespaces(value) {
  if (value[value.length - 1] === ' ') {
    return value.replace(/[^a-zA-Z0-9 .!,]/g, '').slice(0, -1);
  }
  if (value[0] === ' ' && value[1] === ' ') {
    return value.replace(/[^a-zA-Z0-9 .!,]/g, '').slice(2);
  }
  if (value[0] === ' ') {
...
function removeFirstOccurrences(str, value) {
  ...
  a = str.slice(0, b) + str.slice(b + value.length);
  return a;
}
